...
function WorkerApplicationPage() {
  ...
  // Primary service category options (dropdown)
  ...
  const searchAddress = async (query) => {
    try {
      ...
      toast.error("Unable to fetch address suggestions. Please try again.");
    }
  };
  ...
  const validate = () => {
    ...
    if (!formData.experience.trim())
      errs.experience = "Please describe your experience";

    // NEW FIELD VALIDATION
    if (!formData.base_charge || Number(formData.base_charge) <= 0) {
      errs.base_charge =
        "Please enter your typical base charge (per job or visit)";
    }
    if (!formData.selected_service_category) {
      errs.selected_service_category =
        "Please choose your primary service category";
    }
    ...
    if (!formData.aadhaar_card)
      errs.aadhaar_card = "Aadhaar card is required";

    // Payment
    if (!formData.coinsPaid)
      errs.coinsPaid = "You must pay ₹150 before submitting";
  ...
  const handlePayment = async () => {
    if (!formData.name || !formData.email) {
      toast.error(
        "Please enter your full name and email before paying the application fee."
      );
      return;
    }
    ...
    try {
      ...
      tempFormData.append("base_charge", formData.base_charge || "");
      tempFormData.append(
        "selected_service_category",
        formData.selected_service_category || ""
      );

      // Append documents (address_proof removed)
      if (formData.photo_id_path)
        tempFormData.append("photo_id_path", formData.photo_id_path);
      if (formData.aadhaar_card)
        tempFormData.append("aadhaar_card", formData.aadhaar_card);
      if (formData.union_card_path)
        tempFormData.append("union_card_path", formData.union_card_path);
      if (formData.certifications)
        tempFormData.append("certifications", formData.certifications);
      if (formData.signature_copy)
        tempFormData.append("signature_copy", formData.signature_copy);
      ...
      const options = {
        ...
        description: "Pay ₹150 application fee to submit your application",
        handler: async function (response) {
          try {
            ...
            if (verifyResp.status === 200) {
              ...
              toast.success(
                "Payment successful! You can now submit your application form."
              );
            }
          } catch (error) {
            toast.error(
        ...
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-r from-[#0a174e] via-[#161f39] to-[#1a213a] p-6">
          <div className="bg-white rounded-lg shadow-xl max-w-lg w-full p-8 text-center">
            ...
            <p className="text-gray-600 mb-6 text-sm">
              We will review your submission through our 3-stage verification
              process and get back to you via email within 3-5 business days.
            </p>
        ...
        <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full p-8 overflow-auto max-h-screen">
          ...
          <div className="bg-blue-100 border border-blue-300 rounded-lg p-4 mb-6 text-blue-900 text-sm leading-relaxed">
            ...
            <ul className="list-disc list-inside space-y-1">
              ...
              <li>Application fee: <strong>₹150 (one-time)</strong></li>
            ...
            {!paymentSuccess ? (
              <>
                <p className="text-gray-700 mb-3 text-sm">
                  Complete the ₹150 application fee payment before submitting the form (your details are safe).
                </p>
                <button
                ...
                >
                  {loading ? "Processing..." : "💰 Pay ₹150 Application Fee"}
                </button>
              </>
            ) : (
          ...
          <form onSubmit={handleSubmit} className="space-y-5" noValidate>
              ...
              <div className="mb-4 relative">
                ...
                <div className="flex items-center mt-2">
                  ...
                  <p className="text-xs text-gray-600">
                    {selectedLocation
                      ? `📍 Location verified: ${selectedLocation.lat.toFixed(
                          4
            ...
            <div className="border-b pb-4">
              ...
              <div className="mb-4">
                <label
                  htmlFor="selected_service_category"
                ...
                <select
                  ...
                  value={formData.selected_service_category}
                  onChange={handleChange}
                  className={`w-full px-4 py-2 border rounded-md outline-none transition ${
                    errors.selected_service_category ? "border-red-500" : "border-gray-300"
                  } focus:ring-2 focus:ring-blue-400`}
                >
                ...
                {errors.selected_service_category && (
              ...
              {/* Base Charge */}
              <div className="mb-4">
                <label htmlFor="base_charge" className="block text-blue-900 font-semibold mb-1">
                  Typical Base Charge (₹) <span className="text-red-500">*</span>
                </label>
                <input
                  id="base_charge"
                  name="base_charge"
                  ...
                  onChange={handleChange}
                  className={`w-full px-4 py-2 border rounded-md outline-none transition ${
                    errors.base_charge ? "border-red-500" : "border-gray-300"
                  } focus:ring-2 focus:ring-blue-400`}
                  placeholder="E.g., 500"
                />
                {errors.base_charge && (
                  <p className="text-red-500 text-sm mt-1">{errors.base_charge}</p>
                )}
              </div>
            ...
            <div className="border-b pb-4">
              ...
              <div className="mb-4">
                ...
                <input
                  ...
                  onChange={handleChange}
                  className={`w-full ${
                    errors.photo_id_path ? "border-red-500" : "border-gray-300"
                  } rounded-md outline-none transition`}
                />
                <p className="text-xs text-gray-500 mt-1">Upload passport-size photograph</p>
                {errors.photo_id_path && (
                  <p className="text-red-500 text-sm mt-1">{errors.photo_id_path}</p>
                )}
              </div>
              ...
              <div className="mb-4">
                ...
                />
                  ...
                  Front and back (single file or combined PDF) - serves as address proof
                </p>
                {errors.aadhaar_card && (
                  <p className="text-red-500 text-sm mt-1">{errors.aadhaar_card}</p>
                )}
              </div>
              ...
              <div className="mb-4">
                ...
                />
                <p className="text-xs text-gray-500 mt-1">
                  If you're a member of any labor union
                </p>
              </div>
              ...
              <div className="mb-4">
                ...
                />
                <p className="text-xs text-gray-500 mt-1">
                  Trade certificates, training completion documents
                </p>
              </div>
            ...
            <div className="flex items-center space-x-2 bg-gray-50 p-3 rounded-lg">
              ...
              <label htmlFor="coinsPaid" className="block text-blue-900 font-semibold">
                {formData.coinsPaid ? "✅ ₹150 application fee received" : "⚠️ Payment pending"}
              </label>
            </div>
            {errors.coinsPaid && (
              <p className="text-red-500 text-sm">{errors.coinsPaid}</p>
            )}
