...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('email', 'name', 'first_name', 'last_name', 'address', 'phone', 'location', 'date_joined')
...
class WorkerSettingsSerializer(serializers.ModelSerializer):
    ...
    address = serializers.CharField(required=False, allow_blank=True)  # <-- important
    class Meta:
        model = Worker
    ...
    def update(self, instance, validated_data):
        request = self.context.get('request')
        print("Before update, address:", instance.address)
    

        # Update nested user fields
        user_data = validated_data.pop('user', None)
        if user_data is None and request and 'user' in request.data:
            try:
        ...
        # Update Worker address (always save current address from settings)
        if request and 'address' in request.data:
             
             instance.address = request.data['address']
        print("Updating address:", request.data.get('address')) 
        # Update location
...
class Verifier2ReviewSerializer(serializers.ModelSerializer):
    applicant = serializers.CharField(source='application.name', default='Unknown')
