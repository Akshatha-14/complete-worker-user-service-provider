...
def worker_homepage(request):
    try:
        ...
        settings_data = WorkerDetailedSerializer(worker).data  # <--- This uses the new serializer
...
def update_worker_settings(request):
    ...
    # If multipart/form-data, parse user JSON string into dict
    ...
    serializer = WorkerSettingsSerializer(worker, data=data)
