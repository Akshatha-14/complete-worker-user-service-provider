...
def api_user_login(request):
    try:
        payload = request.data
        print(f"📥 Raw payload: {payload} (type: {type(payload)})")

        # Check for key and data
        key_enc = payload.get('key')
        data_enc = payload.get('data')

        if not key_enc or data_enc is None:
            return Response({"error": "Missing encryption key or data."}, status=400)
        ...
        if not email or not password:
            return Response({"error": "Decryption failed."}, status=400)

        email = email.strip()
        password = password.strip()

        print(f"📧 Decrypted email: {email}, password: ***")
        ...
        # Get role and profile completeness
        user_role = UserRole.objects.filter(user=user).first()
        role = user_role.role if user_role else ("admin" if user.is_staff else "user")
        profile_complete = all([user.phone, user.address, getattr(user, 'location', None)])
...
def recommend_view(request, user_id):
    ...
    # --- Fetch worker images and addresses ---
    ...
    # Create maps for avatar and address
    image_map = {item['id']: item['profile_image_url'] for item in serializer.data}
    address_map = {worker.id: worker.address or "Not provided" for worker in workers}

    # --- Merge images and addresses into recommendations ---
    for rec in recommendations:
        if isinstance(rec, dict):
            worker_id = rec.get("worker_id")
            rec["avatar"] = image_map.get(worker_id)
            rec["address"] = address_map.get(worker_id)
...
def user_profile(request):
    ...
    if request.method == 'GET':
        ...
        return Response({
            ...
            "address": user.address if user.address else "Address not provided",
        ...
        if 'location' in decrypted:
            ...
            if isinstance(loc, dict) and loc.get('type') == 'Point':
                ...
                if (
                    ...
                    # GEOSPoint(x, y) => (longitude, latitude)
...
def accept_job(request):
    ...
    try:
        ...
        if Booking.objects.filter(worker=worker, status='in_progress').exists():
            logger.debug(f'Worker {worker.id} already has an active job.')
            return Response({'detail': 'You already have an active job.'}, status=status.HTTP_400_BAD_REQUEST)

        # Lock the job row to prevent concurrent accepts
        try:
            job = Booking.objects.select_for_update().get(pk=job_id, status='booked')
        except Booking.DoesNotExist:
            logger.debug(f'Booking {job_id} not found or not available.')
        ...
        job.save()
        logger.debug(f'Job {job.id} assigned to worker {worker.id}')

        # Update worker availability
        worker.is_available = False
        worker.save()
        logger.debug(f'Worker {worker.id} set as unavailable')

        serializer = JobSerializer(job)
        return Response(serializer.data, status=status.HTTP_200_OK)

    except Worker.DoesNotExist:
        logger.error(f'Worker not found for user {user.id}')
        return Response({'detail': 'Worker not found.'}, status=status.HTTP_404_NOT_FOUND)

    except Exception as e:
        logger.error(f'Unexpected error in accept_job: {e}', exc_info=True)
...
class WorkerSettingsView(APIView):
    ...
    def put(self, request):
        ...
        # Collect clean data without deepcopy
        ...
        if "user" in data:
            try:
                ...
                return Response({"error": "Invalid user JSON"}, status=400)
        ...
        if new_image:
            ...
            if worker.profile_image and worker.profile_image.name:
                old_path = worker.profile_image.path
                if os.path.exists(old_path):
                    os.remove(old_path)
            worker.profile_image = new_image

        # -------------------------
        # 4. Update Worker fields
        # -------------------------
        if "address" in data:
        ...
        if "allows_cod" in data:
            worker.allows_cod = data["allows_cod"]
        ...
        # -------------------------
        if user_data:
            worker.user.email = user_data.get("email", worker.user.email)
            worker.user.phone = user_data.get("phone", worker.user.phone)
            worker.user.save()

        worker.save()

        serializer = WorkerSettingsSerializer(worker, context={"request": request})
        return Response(serializer.data)
...
class WorkerViewSet(viewsets.ModelViewSet):
    ...
    pagination_class = LimitOffsetPagination  # page_size = 50
    ...
    def get_queryset(self):
        ...
        if query:
            queryset = queryset.filter(
                ...
                Q(service__servicetype__icontains=query) |
